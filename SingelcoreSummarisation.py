...
while alpha<35:
	...
	for cluster in os.listdir('.'):
		...
		filenum += 1
		filename = 'Temp/Summaryoutput' + str(filenum) + '.txt'
		...
		for i in Summary:
			outfile.write(i+'\n')
		outfile.close()
		output = subprocess.check_output("java -cp C_Rouge/C_ROUGE.jar executiverouge.C_ROUGE Temp/Summaryoutput"+str(filenum)+".txt DUC-2004/Test_Summaries/"+cluster+"/ 1 B R",shell=True)
		output = float(output)
		output1 = subprocess.check_output("java -cp C_Rouge/C_ROUGE.jar executiverouge.C_ROUGE Temp/Summaryoutput"+str(filenum)+".txt DUC-2004/Test_Summaries/"+cluster+"/ 1 B F",shell=True)
